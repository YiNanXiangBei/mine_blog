        ...
        if auth_token:
            ...
            if not auth_token_arr or len(auth_token_arr) == 3:
                ...
                if auth_header['typ'] != 'JWT':
                    ...
                    result = response.return_message('', Message.TOKEN_INVALID.value, Code.UNAUTHORIZED.value)
                else:
                    payload = Verificate.decode_auth_token(auth_token)
                    if not isinstance(payload, str):
                        users = SysUser.get_info_by_id(payload['data']['id'])
                        if users is None:
                            result = response.return_message('', Message.NOT_FOUND_USER.value, Code.UNAUTHORIZED.value)
                        else:
